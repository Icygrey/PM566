...
library(data.table)
library(tidyverse)
met<- fread("/Users/huangtao/Desktop/PM566+R/met.gz")
met
# Download the data
...
>fifelse  #faster + ifelse   /function (test, yes, no, na = NA) 
>DT[i,j,by]    #data.table框架:  DT[选择的row，对这i这部分数据进行的function操作，应用function的group]
...
# Removing duplicates
stations[, n := 1:.N, by = .(USAF)]    #.N表示最后一行，so 1:.N 代表选中全部row。 # by=.(USAF)表示对listUSAF进行排序。
stations <- stations[n == 1,][, n := NULL]      #[][] 就是相当于[]&[] ---“and”  #删去n这一行
...
met2<-merge(x=met,y=stations,by.x = "USAFID",by.y = "USAF",all.x = TRUE,all.y = FALSE)


#print out a sample of the data.
met2[1:5, .(USAFID,WBAN,STATE)]
...
met_stations<-met2[,.(wind.sp= mean(wind.sp,na.rm=TRUE),
...
met_stations<-met2[,.(wind.sp= mean(wind.sp,na.rm=TRUE),
                      temp= mean(temp,na.rm=TRUE),
                      atm.press= mean(atm.press,na.rm=TRUE)),by=.(USAFID,STATE)
                      ]
...
```{r}
met_stations<-unique(met2[,.(USAFID,STATE,lon,lat)])

#choose the NO.1 in each STATE/ USAFID----the number represents the stations in STATE
met_stations[,n:=1:.N,by=USAFID]     #sometimes, there are more than one row in the same station, so that, we just pick one.
met_stations<-met_stations[n==1]     # delete the replicated one.
#shortcut way:
#met_stations[,.SD[1],by=USAFID]   ##.SD[1]  ---extract the NO.1 row within groups of USAFID.


#midpoint
met_stations[,lat_mid:= quantile(lat,probs = .5,na.rm = TRUE),by=.(STATE)]
met_stations[,lon_mid:=  quantile(lat,probs = .5,na.rm = TRUE),by=.(STATE)]
met_stations


#looking at the distances
met_stations[, distance:=sqrt((lat - lat_mid)^2 + (lon-lon_mid)^2)]
met_stations[, minrecord:=which.min(distance),by= STATE] #choose the min distance between midpoint location and actual locaation.
...
```{R q3_merging_lat_lon}
all_stations<-met2[,.(USAFID,lat,lon,STATE)][,.SD[1],by="USAFID"]
...
#combine datasets
dat <-met_location[,.(lon,lat)]
dat[,type := "center of the state"]
