...
*use this api:NCBI provides several public APIs that allow programmatic access to many databases and tools.*
...
counts <- xml2::xml_find_first(website, 
                 "/html/body/main/div[9]/div[2]/div[2]/div[1]/span")    #XPath...copy it from website (eg.inspect 300results)


# Turning it(nodes) into text
counts <- as.character(counts)

# Extracting the data using regex
stringr::str_extract(counts, "[0-9,]+")    #Extract matching patterns from a string.  #pattern: [regular expression]
```
...
# Find all the ids 
ids <- stringr::str_extract_all(ids, "<Id>[0-9]+</Id>")#[[1]]   #extract


# Remove all the leading and trailing <Id> </Id>. Make use of "|"     #remove
...
2. Query parameters:
- db: pubmed
- id: A character with all the ids separated by comma, e.g., "1232131,546464,13131" *('paste(ids,collapse=",")')*
- retmax: 1000
- rettype: abstract
...
institution <- str_extract_all(
  publications_txt,
  "University of\\s[[:alpha:]]+|[[:alpha:]]+ Institute of [[:alpha:]]+"    #\\s is space
  ) 

institution <- unlist(institution)   #unlist => vectors
table(institution)
```
...
schools_and_deps <- str_extract_all(
  publications_txt,       #tolower()
  "School of\\s[[:alpha:]]+|Department of [[:alpha:]]+"    #\\s is space
  ) 
...
pub_char_list <- xml2::xml_children(publications)      #doc  -> xml_nodset
